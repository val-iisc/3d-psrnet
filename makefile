...
all: utils/tf_ops/cd/tf_nndistance_so.so utils/show_3d/render_balls_so.so utils/tf_ops/emd/tf_auctionmatch_so.so utils/tf_ops/emd_approx/tf_approxmatch_so.so
.PHONY : all
...
utils/tf_ops/emd_approx/tf_approxmatch_so.so: utils/tf_ops/emd_approx/tf_approxmatch_g.cu.o utils/tf_ops/emd_approx/tf_approxmatch.cpp
	g++ -std=c++11 utils/tf_ops/emd_approx/tf_approxmatch.cpp utils/tf_ops/emd_approx/tf_approxmatch_g.cu.o -o utils/tf_ops/emd_approx/tf_approxmatch_so.so -shared -fPIC -I $(tensorflow) -lcudart -L $(cudalib) -O2 -D_GLIBCXX_USE_CXX11_ABI=0

utils/tf_ops/emd_approx/tf_approxmatch_g.cu.o: utils/tf_ops/emd_approx/tf_approxmatch_g.cu
	$(nvcc) -D_GLIBCXX_USE_CXX11_ABI=0 -std=c++11 -c -o utils/tf_ops/emd_approx/tf_approxmatch_g.cu.o utils/tf_ops/emd_approx/tf_approxmatch_g.cu -I $(tensorflow) -DGOOGLE_CUDA=1 -x cu -Xcompiler -fPIC -O2 -arch=sm_30
